...
glance(fit)$sigma
# scatter plot 
# plot(totalcost_trans$ERvisits, totalcost_trans$log_totalcost)
...
$\hat{\beta_0^*} = 5.517, \hat{\beta_{1}} = 0.226$ 

The estimate regression model is 

$\hat{logY_{i}} = 5.517 + 0.226X_{i}$ 

__Comments__: There is an increase of 0.226 in logarithm of total cost for every Emergency Room visit. When there is no Emergency Room visit which is `0` in `ERvisit`, the logarithm of total cost is 5.517. 
...
```{r}
fit2 = lm(log_totalcost ~ ERvisits + comp_bin, data = hd_nv)
summary(fit)$coef %>% knitr::kable() %>% kable_styling()
summary(fit2)$coef %>% knitr::kable() %>% kable_styling()
(0.226 - 0.203)/0.226
```
Then, calculate the percentage change in the parameter estimate

$\frac{\mid\beta_{crude} - \beta_{adjusted}\mid}{\mid\beta_{crude}\mid} = \frac{\mid0.226 - 0.203\mid}{\mid0.226\mid} = 0.1017699 \approx 10.18%$

__Conclusion__

According to 10% rule of thumb, since the percentage change is 10.18%, which is slightly greater than 10%, indicates that the association between `log_totalcost` and `ERvisits` may be influenced by `comp_bin` but the confounding effect is not that strong. 
...
Also, `comp_bin` is a potential confounder for the relationship, as the parameter estimate percentage change slightly over 10%. Therefore, we need to include `comp_bin` along with `ERvisits`.
