...
skimr::skim_to_wide(hd_num) %>% kable() %>% kable_styling()
```
...
totalcost_trans = hd_data %>% 
  mutate(totalcost, log_totalcost = log(totalcost + 1)) 
...
_Notes_: As we have three `0` observations in variable `totalcost`, we transform the `totalcost` with $log (x+1)$.
...
$\hat{\beta_0^*} = 5.527, \hat{\beta_{1}} = 0.225$ 

The estimate regression model is 

$\hat{logY_{i}} = 5.527 + 0.225X_{i}$ 
...
# Fit a linear regression with comp_bin and age
fit_multi1 = lm(log_totalcost ~ ERvisits + comp_bin + age, data = hd_nv)
summary(fit_multi1)

# Fit a linear regression with comp_bin and gender
fit_multi2 = lm(log_totalcost ~ ERvisits + comp_bin + gender, data = hd_nv)
summary(fit_multi2)

# Fit a linear regression with comp_bin and duration
fit_multi3 = lm(log_totalcost ~ ERvisits + comp_bin + duration, data = hd_nv)
summary(fit_multi3)

# Fit a linear regression with comp_bin and additional variables
fit_multi4 = lm(log_totalcost ~ ERvisits + comp_bin + duration + gender + age, data = hd_nv)
summary(fit_multi4)
...
From the results above, we can see that, there is a strong significant that coefficient estimator of `duration` is not zero. By adding `duration`, the `Adjusted R-squared` increased from `0.1361` in `fit_multi` to `0.2589` in `fit_multi3`. Therefore, we should adjust the model for `duration`.

For `gender`, we don't have strong evidence to reject coefficient esitmator is zero. Therefore, we will not take it as a predictor. 

We can also adjust the model for `age` as in `fit_multi4`, pval < 0.05 and it's statistically significant that the coefficient estimator is non-zero. However, we may not want to do that because by adding `age` as a predictor, we got lower `Adjusted R-squared` from `0.1361` in `fit_multi` to `0.1356` in `fit_multi1`.
...
First, the `Adjusted R-squared` values are increased by adding these covariates. 

Second, the p-value for each preditor are low enough, which indicates terms are statistically significant. 
